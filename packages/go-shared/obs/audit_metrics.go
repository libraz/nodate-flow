...
// auditWriteFailuresTotal counts audit rows a recorder built but could
...
// non-zero rate is the only sign that a service answered 2xx for an
// action that left no trace. Any sustained value means the audit trail
// is incomplete for that window, for whichever service the job label
// names.
