...
# -v is load-bearing on this target and the four below, unlike the guards
...
# report is visible only on the failing run, which is not the run where an
# unseen gap matters.
...
check-column-bounds: ## Fail when an input bound is wider than the column behind it, when two surfaces bound the same column differently, or when a field landing in a bounded column declares no bound at all
	cd apps/flow-api && NF_TEST_INTEGRATION= go test -count=1 -v ./tests/columnbounds/

check-response-ids: ## Fail when an MCP tool response carries a row's internal id, either under a key or by returning the row whole
	cd apps/flow-api && NF_TEST_INTEGRATION= go test -count=1 -v ./tests/responseids/
...
check-duplicate-refusals: ## Fail when a named duplicate refusal sits on a write whose table has no key a caller can collide, or when the write behind one cannot be resolved
	cd apps/flow-api && NF_TEST_INTEGRATION= go test -count=1 -v ./tests/duplicaterefusal/
