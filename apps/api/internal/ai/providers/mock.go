...
// a "testdata/ai" directory under an "apps/api" ancestor; this lets the
// mock work both from `cd apps/api && go test` and from the repo root.
func resolveFixtureDir(override string) (string, error) {
	if override != "" {
		return override, nil
	}
	if env := os.Getenv("NF_AI_MOCK_FIXTURE_DIR"); env != "" {
		return env, nil
	}
	wd, err := os.Getwd()
	if err != nil {
