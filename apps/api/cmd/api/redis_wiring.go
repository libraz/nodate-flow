...
// buildSessionStore selects the Redis driver when NF_SESSION_STORE=redis
...
// destination when NF_OUTBOUND_BACKEND=redis. Called from main.go
...
func dialRedis(cfg *config.Config, logger *slog.Logger) *redis.Client {
	if cfg.RedisAddr == "" {
		logger.Warn("redis backend requested but NF_REDIS_ADDR is empty; falling back")
		return nil
	}
