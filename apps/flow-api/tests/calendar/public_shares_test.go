...
func TestRenderPublicShare_HappyPath(t *testing.T) {
	...
	helpers.DoJSON(t, http.MethodPost, tt.WsPath("calendars", calID, "events"), tt.AccessToken, map[string]any{
		...
		"startAt":    time.Date(2026, 5, 1, 10, 0, 0, 0, time.UTC).Unix(),
		"endAt":      time.Date(2026, 5, 1, 11, 0, 0, 0, time.UTC).Unix(),
		"timezone":   "UTC",
		"visibility": "default",
	}, &evt)
...
func TestAttachEvents_RejectsConfidential(t *testing.T) {
	...
	helpers.DoJSON(t, http.MethodPost, tt.WsPath("calendars", calID, "events"), tt.AccessToken, map[string]any{
		...
		"startAt":    time.Date(2026, 5, 2, 9, 0, 0, 0, time.UTC).Unix(),
		"endAt":      time.Date(2026, 5, 2, 10, 0, 0, 0, time.UTC).Unix(),
		"timezone":   "UTC",
		"visibility": "default",
	}, &publicEvt)
	...
	helpers.DoJSON(t, http.MethodPost, tt.WsPath("calendars", calID, "events"), tt.AccessToken, map[string]any{
		...
		"startAt":    time.Date(2026, 5, 2, 11, 0, 0, 0, time.UTC).Unix(),
		"endAt":      time.Date(2026, 5, 2, 12, 0, 0, 0, time.UTC).Unix(),
		"timezone":   "UTC",
...
func TestDetachEvent_RemovesFromRender(t *testing.T) {
	...
	helpers.DoJSON(t, http.MethodPost, tt.WsPath("calendars", calID, "events"), tt.AccessToken, map[string]any{
		...
		"startAt":    time.Date(2026, 5, 3, 9, 0, 0, 0, time.UTC).Unix(),
		"endAt":      time.Date(2026, 5, 3, 10, 0, 0, 0, time.UTC).Unix(),
		"timezone":   "UTC",
		"visibility": "default",
	}, &evt)
