...
// createEventForTenant creates a single calendar event owned by tt and
// returns the event's public ID.
func createEventForTenant(t *testing.T, tt *helpers.CalendarTestTenant, calID, title string) string {
	t.Helper()
	var evt struct {
		ID string `json:"id"`
	}
...
func addAttendee(t *testing.T, tt *helpers.CalendarTestTenant, calID, evtID, userPublicID string) string {
	t.Helper()
	var resp struct {
		Attendees []struct {
			ID     string `json:"id"`
...
// createdInvite is the shape of POST .../invite. Token is only present
// here; the list endpoint scrubs it.
...
func createInvite(t *testing.T, tt *helpers.CalendarTestTenant, calID, evtID, attendeeID string) createdInvite {
	t.Helper()
...
// expires_at 1 second in the past for the given invite public_id.
...
// (event public_id, user public_id) pair.
...
func listEventInvites(t *testing.T, tt *helpers.CalendarTestTenant, calID, evtID string) []eventInviteListItem {
	t.Helper()
	var resp struct {
