...
func TestPropose_TestOnly(t *testing.T) {
	t.Parallel()
	p := Propose([]Change{
		{Path: "apps/flow-api/internal/ai/judge/judge_test.go", Status: StatusAdded},
...
func TestPropose_MixedScopeDropped(t *testing.T) {
	t.Parallel()
	p := Propose([]Change{
		{Path: "apps/flow-api/internal/foo.go", Status: StatusModified},
...
func TestPropose_DeletionHeavyBecomesFix(t *testing.T) {
	t.Parallel()
	p := Propose([]Change{
		{Path: "apps/flow-api/internal/legacy/a.go", Status: StatusDeleted},
		{Path: "apps/flow-api/internal/legacy/b.go", Status: StatusDeleted},
		{Path: "apps/flow-api/internal/legacy/c.go", Status: StatusDeleted},
		{Path: "apps/flow-api/internal/legacy/d.go", Status: StatusModified},
	}, "")
	if p.Type != "fix" {
...
func TestPropose_SummaryBecomesSubject(t *testing.T) {
	t.Parallel()
	p := Propose([]Change{
		{Path: "apps/flow-api/main.go", Status: StatusModified},
	}, "Upgrade Huma to v2.")
