// Guard + metering tests for the workspace-backed nlcommand provider.
//
// These lock in audit fix C-2 (H-8 / M-6): the command-palette NL
...
// recordingProvider captures the request and returns a canned response or
// error. Implements providers.Provider.
...
// TestResolveCommand_BudgetExhaustedBlocks proves the workspace daily
// budget guard short-circuits the call: when the guard reports the cap is
// hit, ResolveCommand returns ErrBudgetExceeded and never bills the
// provider.
func TestResolveCommand_BudgetExhaustedBlocks(t *testing.T) {
	t.Parallel()
	prov := &recordingProvider{kind: providers.Kind("mock"), resp: &providers.Response{Text: `{"tool":"create_task","args":{},"confidence":1}`}}
	...
	_, err := wp.ResolveCommand(context.Background(), "create a task", nil)
	if !errors.Is(err, ErrBudgetExceeded) {
		t.Fatalf("ResolveCommand error = %v, want ErrBudgetExceeded", err)
	}
	if prov.called {
...
// TestResolveCommand_SuccessLogsInvocationWithCost proves a successful NL
// call routes through the same metering path as the orchestrator: it logs
// one ai_invocations record carrying the model, redacted prompt/response,
// token counts, and cost cents.
func TestResolveCommand_SuccessLogsInvocationWithCost(t *testing.T) {
	t.Parallel()
	prov := &recordingProvider{
		kind: providers.Kind("mock"),
		resp: &providers.Response{
			Text:         `{"tool":"create_task","args":{},"confidence":1}`,
			InputTokens:  31,
			OutputTokens: 12,
			CostCents:    42,
		},
	...
	out, err := wp.ResolveCommand(context.Background(), "create a task", nil)
	if err != nil {
		t.Fatalf("ResolveCommand: %v", err)
	}
	if len(out) == 0 {
		t.Fatal("expected non-empty resolved bytes")
	}
	if len(logged) != 1 {
...
// TestResolveCommand_ProviderErrorLogsFailure proves a provider error
// still produces an ai_invocations row so failed spend / errors remain
// attributable.
func TestResolveCommand_ProviderErrorLogsFailure(t *testing.T) {
	t.Parallel()
	...
	if _, err := wp.ResolveCommand(context.Background(), "create a task", nil); err == nil {
