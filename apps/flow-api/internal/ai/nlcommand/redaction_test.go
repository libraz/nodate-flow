// Redaction tests for the workspace-backed nlcommand provider.
//
// The ai_invocations row this path writes is served to every member of
// the workspace, guests included, so a secret a user types into the
// command palette must not reach it. What the row carries is the
...
const promptWithSecret = "create a task, auth with " + pastedSecret //#nosec G101 -- synthetic test fixture, never a real key
...
const resolvedCommand = `{"tool":"create_task","args":{},"confidence":1}`

// TestResolveCommand_PromptSecretNeverReachesTheInvocation pins the scrub
// on the prompt the row stores.
//
...
// prose is gone.
func TestResolveCommand_PromptSecretNeverReachesTheInvocation(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			prov := &recordingProvider{
				...
				resp: &providers.Response{Text: resolvedCommand},
				err:  tc.providerErr,
			}
			...
			_, err := wp.ResolveCommand(context.Background(), promptWithSecret, nil)
			...
			if strings.Contains(rec.PromptRedacted, pastedSecret) {
				t.Errorf("the typed secret reached ai_invocations: %q", rec.PromptRedacted)
			}
			...
			if !strings.Contains(rec.PromptRedacted, "create a task") {
