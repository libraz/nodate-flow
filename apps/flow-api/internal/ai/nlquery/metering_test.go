...
const goodLens = `{"filter":{"blocked":{"eq":true}},"sort":[{"field":"priority","dir":"desc"}]}`

// TestCompileLens_BudgetExhaustedBlocks proves the workspace daily budget
// guard short-circuits the call: when the guard reports the cap is hit,
// CompileLens returns ErrBudgetExceeded and never bills the provider.
func TestCompileLens_BudgetExhaustedBlocks(t *testing.T) {
	t.Parallel()
	prov := &recordingProvider{kind: providers.Kind("mock"), resp: &providers.Response{Text: goodLens}}
	...
	_, err := wp.CompileLens(context.Background(), "everything blocked")
	if !errors.Is(err, ErrBudgetExceeded) {
		t.Fatalf("CompileLens error = %v, want ErrBudgetExceeded", err)
	}
	if prov.called {
...
// TestCompileLens_SuccessLogsInvocationWithCost proves a successful NL
// call routes through the same metering path as the orchestrator: it logs
// one ai_invocations record carrying the redacted prompt/response, token
// counts, and cost cents.
func TestCompileLens_SuccessLogsInvocationWithCost(t *testing.T) {
	t.Parallel()
	prov := &recordingProvider{
		kind: providers.Kind("mock"),
		resp: &providers.Response{Text: goodLens, InputTokens: 31, OutputTokens: 12, CostCents: 42},
	}
	...
	out, err := wp.CompileLens(context.Background(), "everything blocked")
	if err != nil {
...
// TestCompileLens_ProviderErrorLogsFailure proves a provider error still
// produces an ai_invocations row so failed spend / errors are
// attributable.
func TestCompileLens_ProviderErrorLogsFailure(t *testing.T) {
	t.Parallel()
	...
	if _, err := wp.CompileLens(context.Background(), "everything blocked"); err == nil {
