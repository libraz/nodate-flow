// Redaction tests for the workspace-backed nlquery provider.
//
...
// compile-lens box must not reach it. What the row carries is the
...
const promptWithSecret = "everything blocked, auth with " + pastedSecret //#nosec G101 -- synthetic test fixture, never a real key
...
// TestCompileLens_PromptSecretNeverReachesTheInvocation pins the scrub on
// the prompt the row stores.
//
...
// prose is gone.
func TestCompileLens_PromptSecretNeverReachesTheInvocation(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			prov := &recordingProvider{
				...
				resp: &providers.Response{Text: goodLens},
				err:  tc.providerErr,
			}
			...
			_, err := wp.CompileLens(context.Background(), promptWithSecret)
			...
			if !strings.Contains(rec.PromptRedacted, "everything blocked") {
