...
var (
	...
	// WS.IMPORT.CONFIG_KEY_UNKNOWN — The import configuration contains a key this source does not accept
	WsImportConfigKeyUnknown = &Spec{Code: "WS.IMPORT.CONFIG_KEY_UNKNOWN", Status: 400, Message: "The import configuration contains a key this source does not accept", Description: "Returned when configJson carries a key outside the set the chosen import source defines. Applies to sources that have a connector; a source without one has not declared its keys yet.", UserAction: "Remove the key, or choose the import source it belongs to."}
	// WS.IMPORT.CONFIG_SECRET_REJECTED — The import configuration must not carry credentials
	WsImportConfigSecretRejected = &Spec{Code: "WS.IMPORT.CONFIG_SECRET_REJECTED", Status: 400, Message: "The import configuration must not carry credentials", Description: "Returned when configJson carries a key whose name indicates a credential (token, secret, password, API key). The column is stored in plaintext, so credentials are refused at the boundary instead.", UserAction: "Remove the credential from the configuration. Connect the external service through its integration settings instead."}
	...
	// WS.SIGNAL.KIND_UNKNOWN — Unknown signal kind
	WsSignalKindUnknown = &Spec{Code: "WS.SIGNAL.KIND_UNKNOWN", Status: 422, Message: "Unknown signal kind", Description: "Returned when the request supplies a `kind` value that is not present in the signal_kinds/*.yaml registry. The registry is the closed enumeration of signal kinds the system observes (ADR 0008 D2).", UserAction: "Use one of the documented signal kinds (e.g. discord.presence, slack.presence, calendar.event_day_arrived, manual). Adding a new kind requires a YAML entry plus a codegen run."}
	// WS.SIGNAL.SUBJECT_MISMATCH — Signal subject does not match the linked task
	WsSignalSubjectMismatch = &Spec{Code: "WS.SIGNAL.SUBJECT_MISMATCH", Status: 422, Message: "Signal subject does not match the linked task", Description: "Returned when the request supplies both the legacy `taskId` parameter and a new `(subjectType, subjectId)` pair that point at different tasks. The two parameters must address the same task or the new fields must be omitted.", UserAction: "Either send only `taskId`, or send `subjectType='task'` with `subjectId` equal to the same task public id."}
	// WS.SIGNAL.SUBJECT_NOT_FOUND — Signal subject does not exist in this workspace
	WsSignalSubjectNotFound = &Spec{Code: "WS.SIGNAL.SUBJECT_NOT_FOUND", Status: 404, Message: "Signal subject does not exist in this workspace", Description: "Returned when `(subjectType, subjectId)` references a row that does not exist or is not visible in the actor's workspace. Returns 404 to avoid leaking cross-tenant existence.", UserAction: "Verify the subject id, or omit `subjectType`/`subjectId` to fall back to the kind's default subject."}
	...
	// WS.TASK.ACTOR_ROLE_INVALID — Task actor role must be one of: assignee, reviewer, watcher, approver
	WsTaskActorRoleInvalid = &Spec{Code: "WS.TASK.ACTOR_ROLE_INVALID", Status: 422, Message: "Task actor role must be one of: assignee, reviewer, watcher, approver", Description: "Returned when a task create or update payload includes an `actors[].role` value outside the allowed enum. The persisted column is `task_actors.role ENUM('assignee','reviewer','watcher','approver')`, so any other string is rejected at the validation boundary before the SQL layer.", UserAction: "Set each actor's role to one of: assignee, reviewer, watcher, approver."}
	...
	// WS.TASK.DEPENDENCY_CYCLE — Adding this dependency would create a cycle
	WsTaskDependencyCycle = &Spec{Code: "WS.TASK.DEPENDENCY_CYCLE", Status: 422, Message: "Adding this dependency would create a cycle", Description: "Returned when a new task dependency edge would close a cycle in the workspace dependency graph (directly or transitively). The dependency graph must remain a DAG so blocked-by computation and topological scheduling terminate.", UserAction: "Remove the conflicting dependency on the other task before adding this edge."}
	...
	WsTaskAgentCostCapExceeded = &Spec{Code: "WS.TASK_AGENT.COST_CAP_EXCEEDED", Status: 422, Message: "Agent monthly cost cap has been reached", Description: "Returned when starting or resuming an agent run would push the agent's accumulated spend past ai_agents.monthly_cost_cap_cents for the current billing month.", UserAction: "Wait for the next billing cycle, raise the agent's monthly_cost_cap_cents in agent settings, or hand the task off to another agent or user."}
	// WS.TASK_AGENT.HANDOFF_LOOP_DETECTED — Handoff bounced between agent and user too many times
