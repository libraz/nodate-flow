...
var (
	...
	// AI.COST.GUARD_EXCEEDED — Daily AI spend limit reached
	AiCostGuardExceeded = &Spec{Code: "AI.COST.GUARD_EXCEEDED", Status: 402, Message: "Daily AI spend limit reached", Description: "Returned when the workspace has reached its daily AI spend cap for today. The cap is set by the NF_FLOW_AI_DAILY_BUDGET_CENTS deployment setting and is not editable from workspace settings.", UserAction: "Wait until tomorrow when the daily limit resets, or ask your administrator to raise the deployment's daily AI budget."}
	...
	// AI.PROVIDER.BASE_URL_NOT_ALLOWED — AI provider endpoint is not an allowed destination
	AiProviderBaseUrlNotAllowed = &Spec{Code: "AI.PROVIDER.BASE_URL_NOT_ALLOWED", Status: 412, Message: "AI provider endpoint is not an allowed destination", Description: "Returned when an AI provider's base URL points at a destination this deployment refuses to contact: a loopback, private, or link-local address (including cloud metadata endpoints), or a URL that is not an absolute http/https address. Private destinations are refused by default because the base URL is workspace-admin input and the response comes back to the caller.", UserAction: "Use a public https endpoint. To reach a local model server (Ollama, LM Studio), ask your administrator to set NF_FLOW_AI_ALLOW_PRIVATE=1 on the deployment."}
	...
	// AI.PROVIDER.NOT_FOUND — AI provider not found
	AiProviderNotFound = &Spec{Code: "AI.PROVIDER.NOT_FOUND", Status: 404, Message: "AI provider not found", Description: "Returned when an update or delete targets an ai_providers row that does not exist in the workspace, matching zero rows.", UserAction: "Verify the provider id and that it belongs to this workspace."}
	...
	// AI.REVERSE.ALREADY_REVERSED — Event already has a compensating reverse.
	AiReverseAlreadyReversed = &Spec{Code: "AI.REVERSE.ALREADY_REVERSED", Status: 409, Message: "Event already has a compensating reverse.", Description: "An event can only be reversed once. The compensating reverse event is the canonical undo; further changes require new actions.", UserAction: "View the timeline to see the reversal record."}
	// AI.REVERSE.NOT_LLM_ORIGIN — Cannot reverse an event that did not originate from an LLM agent.
	AiReverseNotLlmOrigin = &Spec{Code: "AI.REVERSE.NOT_LLM_ORIGIN", Status: 403, Message: "Cannot reverse an event that did not originate from an LLM agent.", Description: "Only events with actor_agent_id IS NOT NULL may be reversed via the reverse endpoint. User-originated and worker-emitted events are immutable from the user-facing reversal flow.", UserAction: "Edit the source data directly or use the canonical user-facing action to undo it."}
	// AI.REVERSE.TARGET_NOT_FOUND — Event not found in this workspace.
	AiReverseTargetNotFound = &Spec{Code: "AI.REVERSE.TARGET_NOT_FOUND", Status: 404, Message: "Event not found in this workspace.", Description: "The event_public_id was not found among workspace events, or it was deleted via cascade.", UserAction: "Refresh the timeline and retry."}
