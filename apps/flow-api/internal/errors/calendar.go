...
var (
	...
	// CALENDAR.ATTENDEE.NOT_FOUND — This user is not an attendee of this event
	CalendarAttendeeNotFound = &Spec{Code: "CALENDAR.ATTENDEE.NOT_FOUND", Status: 404, Message: "This user is not an attendee of this event", Description: "Returned when the target user is not on the specified event's attendee list, so there is no RSVP or edit permission to change.", UserAction: "Verify the user is on this event's attendee list, and invite them to the event first if they are not."}
	...
	// CALENDAR.EVENT.ALREADY_OCCURRENCE_OVERRIDE — This event already replaces a single occurrence and cannot be divided further
	CalendarEventAlreadyOccurrenceOverride = &Spec{Code: "CALENDAR.EVENT.ALREADY_OCCURRENCE_OVERRIDE", Status: 422, Message: "This event already replaces a single occurrence and cannot be divided further", Description: "Returned when a request uses the occurrence or thisAndFollowing scope on an override row. An override stands for exactly one occurrence of its series and produces none of its own.", UserAction: "Address the recurring event this row replaces an occurrence of, or edit this row itself with the series scope."}
	...
	// CALENDAR.EVENT.END_BEFORE_START — endAt must be at or after startAt
	CalendarEventEndBeforeStart = &Spec{Code: "CALENDAR.EVENT.END_BEFORE_START", Status: 422, Message: "endAt must be at or after startAt", Description: "Returned when a create or patch request supplies an endAt earlier than its startAt. The same ordering is enforced by the chk_calendar_events_chronology database constraint, so an unchecked request would surface as an unexplained server error instead.", UserAction: "Send an endAt at or after startAt. A zero-length event (endAt equal to startAt) is accepted and renders as a milestone."}
	...
	// CALENDAR.EVENT.NOT_RECURRING — This event does not repeat, so it has no single occurrence to act on
	CalendarEventNotRecurring = &Spec{Code: "CALENDAR.EVENT.NOT_RECURRING", Status: 422, Message: "This event does not repeat, so it has no single occurrence to act on", Description: "Returned when a request uses the occurrence or thisAndFollowing scope on an event that carries no recurrence rule.", UserAction: "Use the series scope, or add a recurrence rule before addressing a single occurrence."}
	// CALENDAR.EVENT.OCCURRENCE_START_REQUIRED — occurrenceStart is required when the request targets a single occurrence
	CalendarEventOccurrenceStartRequired = &Spec{Code: "CALENDAR.EVENT.OCCURRENCE_START_REQUIRED", Status: 422, Message: "occurrenceStart is required when the request targets a single occurrence", Description: "Returned when a request uses the occurrence or thisAndFollowing scope without an occurrenceStart. The original start of the occurrence is what identifies which one the request means.", UserAction: "Send the occurrence's original start as occurrenceStart, or use the series scope to act on every occurrence."}
	...
	// CALENDAR.EVENT.RECURRENCE_ON_OCCURRENCE_NOT_ALLOWED — A single occurrence cannot carry its own recurrence rule, recurrence end or exception list
	CalendarEventRecurrenceOnOccurrenceNotAllowed = &Spec{Code: "CALENDAR.EVENT.RECURRENCE_ON_OCCURRENCE_NOT_ALLOWED", Status: 422, Message: "A single occurrence cannot carry its own recurrence rule, recurrence end or exception list", Description: "Returned when a request sets or clears recurrenceRule, recurrenceEnd or recurrenceExceptions on a single occurrence. Those fields describe the series and belong to the recurring event alone.", UserAction: "Send recurrence changes to the recurring event with the series scope."}
