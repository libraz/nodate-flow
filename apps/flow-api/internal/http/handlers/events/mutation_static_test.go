...
// reverseAppendOwner is the one file allowed to append a compensating
// event, and eventbus.AppendReverseEvent is the one entry point it may
// use.
//
// The exception is not a convenience. The compensating row and the state
// rollback that accompanies it have to commit together, and the UNIQUE
// (workspace_id, reverses_event_id) index is what turns the pair into a
// single winner when two requests undo the same event at once. Appending
// that row through the recorder would take it out of the transaction and
// lose both properties, so the append stays where it is and the recorder
// supplies the half it cannot: the audit row.
const reverseAppendOwner = "reverse.go"
...
// ownedRawAppends are checked against [reverseAppendOwner] instead: no
// other file may reference them, and the owner must keep referencing
// them, so a guard whose subject has moved elsewhere says so rather than
// passing on a rule that no longer applies.
var ownedRawAppends = []string{
...
// [TestEventsMutatingOperationsRecordBothHalves] until it either routes
...
func loadEventsPackage(t *testing.T) *mutationguard.Analysis {
	t.Helper()
...
// TestEventsMutatingOperationsRecordBothHalves walks the registered
...
// names on purpose: the fix for an operation that is wrong today is not a
// fix, because the next one is written by somebody who never read this
// file.
func TestEventsMutatingOperationsRecordBothHalves(t *testing.T) {
	t.Parallel()

	a := loadEventsPackage(t)
...
// TestEventsRecordsOnlyThroughTheMutationLog proves no handler reaches
// around the recorder to either log on its own, save for the one append
// [reverseAppendOwner] owns.
//
...
// is how a reversal came to be visible on the timeline while no audit
// query could name who performed it.
func TestEventsRecordsOnlyThroughTheMutationLog(t *testing.T) {
	t.Parallel()
	...
	for _, f := range a.Centralized(reverseAppendOwner, ownedRawAppends) {
		t.Error(f.String())
	}
...
// TestEventsMutationLiteralsNameTheWholeChange reads every mutation
...
func TestEventsMutationLiteralsNameTheWholeChange(t *testing.T) {
	t.Parallel()
