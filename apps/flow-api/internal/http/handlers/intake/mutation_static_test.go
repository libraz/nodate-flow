...
// [TestIntakeMutatingOperationsRecordBothHalves] until it either routes
...
func loadIntakePackage(t *testing.T) *mutationguard.Analysis {
	t.Helper()
...
// TestIntakeMutatingOperationsRecordBothHalves walks the registered
...
// this file.
func TestIntakeMutatingOperationsRecordBothHalves(t *testing.T) {
	t.Parallel()

	a := loadIntakePackage(t)
...
// TestIntakeRecordsOnlyThroughTheMutationLog proves no handler reaches
...
// which is the state most of this application's handlers are in. Keeping
// both writes behind one call is what makes "event and audit row
// together" a property of the package rather than a habit.
func TestIntakeRecordsOnlyThroughTheMutationLog(t *testing.T) {
	t.Parallel()
...
// TestIntakeMutationLiteralsNameTheWholeChange reads every mutation
...
func TestIntakeMutationLiteralsNameTheWholeChange(t *testing.T) {
	t.Parallel()
