...
// package may reach. An append written beside its own audit entry is two
// descriptions of one change that can drift apart, and an append written
// without one puts the change on the timeline where no audit query can
// find it.
...
// [TestTimeboxMutatingOperationsRecordBothHalves] until it either routes
...
func loadTimeboxPackage(t *testing.T) *mutationguard.Analysis {
	t.Helper()
...
// TestTimeboxMutatingOperationsRecordBothHalves walks the registered
...
// file.
func TestTimeboxMutatingOperationsRecordBothHalves(t *testing.T) {
	t.Parallel()

	a := loadTimeboxPackage(t)
...
// TestTimeboxRecordsOnlyThroughTheMutationLog proves no handler reaches
// around the recorder to either log on its own.
//
// Without this half, the reachability test above is satisfiable by a
// handler that appends its event directly and skips the audit row, or by
// one that writes the two beside each other under conditions that do not
// match — the shape where the timeline carries a change an audit query
// cannot find.
func TestTimeboxRecordsOnlyThroughTheMutationLog(t *testing.T) {
	t.Parallel()
...
// TestTimeboxMutationLiteralsNameTheWholeChange reads every mutation
...
func TestTimeboxMutationLiteralsNameTheWholeChange(t *testing.T) {
	t.Parallel()
