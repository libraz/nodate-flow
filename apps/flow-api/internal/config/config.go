...
type Config struct {
	Port     string `env:"ND_FLOW_PORT" envDefault:"8080"`
	LogLevel string `env:"ND_FLOW_LOG_LEVEL" envDefault:"info"`
	...
	GhWebhookSecret string `env:"ND_FLOW_GH_WEBHOOK_SECRET" envDefault:""`
	// SlackSigningSecret is the v0 signing secret used to verify inbound
	// Slack event deliveries.
	SlackSigningSecret string `env:"ND_FLOW_SLACK_SIGNING_SECRET" envDefault:""`
	...
	GoogleChannelToken string `env:"ND_FLOW_GOOGLE_CHANNEL_TOKEN" envDefault:""`
	...
	DefaultWorkspaceID string `env:"ND_FLOW_DEFAULT_WORKSPACE_ID" envDefault:""`
	...
	PublicBaseURL string `env:"ND_FLOW_PUBLIC_URL" envDefault:"http://localhost:8080"`

	// WebBaseURL is where the user is redirected after the OAuth
	// callback handler completes. Defaults to the Vite dev origin.
	WebBaseURL string `env:"ND_FLOW_WEB_URL" envDefault:"http://localhost:5173"`
	...
	GithubClientID     string `env:"ND_FLOW_GITHUB_CLIENT_ID"      envDefault:""`
	GithubClientSecret string `env:"ND_FLOW_GITHUB_CLIENT_SECRET"  envDefault:""`
	SlackClientID      string `env:"ND_FLOW_SLACK_CLIENT_ID"       envDefault:""`
	SlackClientSecret  string `env:"ND_FLOW_SLACK_CLIENT_SECRET"   envDefault:""`
	GoogleClientID     string `env:"ND_FLOW_GOOGLE_CLIENT_ID"      envDefault:""`
	GoogleClientSecret string `env:"ND_FLOW_GOOGLE_CLIENT_SECRET"  envDefault:""`
	...
	// deployment must explicitly set ND_COOKIE_SECURE=true so the
	...
	AiMock bool `env:"ND_FLOW_AI_MOCK" envDefault:"false"`
	...
	// the box; set ND_FLOW_STREAM=false to disable (useful for load tests
	// or when running without the SSE-aware web client).
	StreamEnabled bool `env:"ND_FLOW_STREAM" envDefault:"true"`
	...
	CorsAllowedOrigins []string `env:"ND_FLOW_CORS" envSeparator:"," envDefault:"http://localhost:5173,http://127.0.0.1:5173"`
	...
	// or "redis" (requires ND_REDIS_ADDR).
	StreamBackend string `env:"ND_FLOW_STREAM_BACKEND" envDefault:"memory"`
	// OutboundBackend selects the egress rate limiter driver: "memory"
	// (default) or "redis" (requires ND_REDIS_ADDR).
	OutboundBackend string `env:"ND_FLOW_OUTBOUND_BACKEND" envDefault:"memory"`
	...
	AgentTickInterval time.Duration `env:"ND_FLOW_AGENT_TICK_INTERVAL" envDefault:"1m"`
	...
	AgentQueueBackend string `env:"ND_FLOW_AGENT_QUEUE_BACKEND" envDefault:"memory"`
	...
	AgentWorkerCount int `env:"ND_FLOW_AGENT_WORKER_COUNT" envDefault:"1"`
	...
	AgentRunner string `env:"ND_FLOW_AGENT_RUNNER" envDefault:"orchestrator"`
	...
	AgentRunsPurgeInterval time.Duration `env:"ND_FLOW_AGENT_RUNS_PURGE_INTERVAL" envDefault:"1h"`
	// AgentRunsRetention is the minimum age a finished agent_runs
	// row must reach before the purger deletes it.
	AgentRunsRetention time.Duration `env:"ND_FLOW_AGENT_RUNS_RETENTION" envDefault:"168h"`
	...
	AutoActionInterval time.Duration `env:"ND_FLOW_AUTO_ACTION_INTERVAL" envDefault:"5m"`
	// AutoActionThreshold is the minimum confidence score for an
	// auto-action to be applied without human approval. Default: 0.80.
	AutoActionThreshold float64 `env:"ND_FLOW_AUTO_ACTION_THRESHOLD" envDefault:"0.80"`
	...
	AutoActionDryRun bool `env:"ND_FLOW_AUTO_ACTION_DRY_RUN" envDefault:"false"`
	...
	OtelEndpoint string `env:"ND_FLOW_OTEL_ENDPOINT" envDefault:""`
	// OtelInsecure disables TLS for the OTLP exporter connection.
	// Useful for local development against a sidecar collector.
	OtelInsecure bool `env:"ND_FLOW_OTEL_INSECURE" envDefault:"true"`

	// SmtpHost is the SMTP server hostname. When empty, email
	// notifications are disabled.
	SmtpHost string `env:"ND_FLOW_SMTP_HOST" envDefault:""`
	// SmtpPort is the SMTP server port (typically 587 for STARTTLS).
	SmtpPort int `env:"ND_FLOW_SMTP_PORT" envDefault:"587"`
	// SmtpUsername is the SASL login. Empty for unauthenticated relays.
	SmtpUsername string `env:"ND_FLOW_SMTP_USERNAME" envDefault:""`
	// SmtpPassword is the SASL secret.
	SmtpPassword string `env:"ND_FLOW_SMTP_PASSWORD" envDefault:""`
	// SmtpFrom is the default envelope sender address.
	SmtpFrom string `env:"ND_FLOW_SMTP_FROM" envDefault:"noreply@nodate-flow.local"`
	...
	// provider.
	EmbedOpenAIKey string `env:"ND_FLOW_EMBED_OPENAI_KEY" envDefault:""`
