...
// via the constraint primitives, with no new DB tables.
//
// Sprint — a sprint is just a collection of tasks that must all be
// completed (dependency.all_done) before a target cutoff
// (time.due_before). No `sprints` or `cycles` table is required.
//
// Goal — a goal is just the dependency.all_done of its child tasks.
// A task belongs to a goal iff it appears in the goal task's
// dependency list. No `goals` table is required.
...
// done. A "goal" is nothing more than a parent task whose
