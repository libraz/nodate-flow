...
/* ============================================
   useOutsideClose — click-outside + Escape
   ============================================ */
...
function DatePickerPanel({
  ...
  const monthLabel = viewMonth.setLocale(locale).toLocaleString({ month: 'long', year: 'numeric' });
...
/* ============================================
   TimePicker — scrollable 24h time slots
   ============================================ */
...
export function CustomSelect({
  ...
  const { anchorRef, floatingRef, pos } = useFloating(open, 200);
  const selectedRef = useRef<HTMLButtonElement>(null);
            ...
            {options.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`flex items-center gap-2 ${isSelected ? '' : 'pl-[22px]'}`}>
                    {opt.icon}
                    {opt.label}
                  </span>
                </button>
...
export function DatePickerDropdown({
  ...
  const floating = useFloating(showPicker, 280);
  ...
  return (
    <div className={className}>
      ...
      {showPicker && (
        <DatePickerPanel
          value={dt.isValid ? dt : DateTime.now()}
          onChange={(d) => {
            onChange(d.toFormat('yyyy-MM-dd'));
            handleClose();
          }}
          onClose={handleClose}
          style={{ top: floating.pos.top, left: floating.pos.left }}
...
export function TimePickerDropdown({
  ...
  const [showPicker, setShowPicker] = useState(false);
  const floating = useFloating(showPicker, 100);
  const handleClose = useCallback(() => setShowPicker(false), []);
