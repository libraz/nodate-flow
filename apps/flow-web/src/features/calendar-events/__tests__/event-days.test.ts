...
/** Unix seconds for a local wall-clock moment. */
function local(y: number, m: number, d: number, hh = 0, mm = 0): number {
...
describe('eventDayKeys', () => {
  it('gives a single-day event one key', () => {
    expect(
      eventDayKeys({
        startAt: local(2027, 6, 1, 10),
        endAt: local(2027, 6, 1, 11),
      } as never),
    ).toEqual(['2027-06-01']);
  });
  ...
  it('gives a multi-day event one key per day it covers', () => {
    expect(
      eventDayKeys({
        startAt: local(2027, 6, 1, 9),
        endAt: local(2027, 6, 5, 17),
      } as never),
  ...
  it('includes both ends of a two-day event', () => {
    expect(
      eventDayKeys({
        startAt: local(2027, 6, 1, 22),
        endAt: local(2027, 6, 2, 1),
      } as never),
  ...
  it('spans a month boundary', () => {
    expect(
      eventDayKeys({
        startAt: local(2027, 5, 30, 9),
        endAt: local(2027, 6, 2, 9),
      } as never),
  ...
  it('treats a missing end as a single day', () => {
    expect(eventDayKeys({ startAt: local(2027, 6, 1, 10) } as never)).toEqual(['2027-06-01']);
  });

  it('returns nothing for an undated event', () => {
    expect(eventDayKeys({} as never)).toEqual([]);
  });
  ...
  it('does not run backwards when the end precedes the start', () => {
    expect(
      eventDayKeys({
        startAt: local(2027, 6, 5, 10),
        endAt: local(2027, 6, 1, 10),
      } as never),
    ).toEqual(['2027-06-05']);
  });
  ...
  it('caps a runaway span', () => {
    const keys = eventDayKeys({
      startAt: local(2027, 6, 1),
      endAt: local(2099, 6, 1),
    } as never);
