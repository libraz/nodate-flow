...
describe('logError', () => {
  it('forwards to console.error in dev with the [flow-web] prefix', () => {
    vi.stubEnv('DEV', true);
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const err = new Error('boom');
    logError('Failed to fetch sessions', err);
    expect(spy).toHaveBeenCalledTimes(1);
    expect(spy).toHaveBeenCalledWith('[flow-web] Failed to fetch sessions', err);
  });
  ...
  it('accepts a missing err argument without throwing', () => {
    ...
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    logError('label only');
    expect(spy).toHaveBeenCalledWith('[flow-web] label only', undefined);
  });
});
