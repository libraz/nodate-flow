 ...
 * renamed parameter, a changed query serializer, a different error
 * envelope. Those are exactly the breakages the OpenAPI → SDK codegen
 * chain is supposed to catch, and mocking the SDK is precisely the
 * layer that hides them. Going through MSW means the assertions see
 * the URL, the headers, and the body that would have gone to the API.
 *
...
/**
 * Base URLs the app's two SDK clients point at under test. They mirror
 * the defaults in `src/lib/sdk.ts` so handlers can be written against
 * the same origins the client will actually call.
 */
