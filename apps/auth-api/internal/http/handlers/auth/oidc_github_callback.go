...
func OIDCGithubCallback(deps Deps) func(context.Context, *OIDCCallbackInput) (*OIDCCallbackOutput, error) {
	return func(ctx context.Context, in *OIDCCallbackInput) (*OIDCCallbackOutput, error) {
		...
		// consumeOIDCState for what the gate covers. GitHub itself does
		// not bind a nonce yet, but threading it through removes drift
		// between handlers and prepares for a future when it does.
		nonce, err := deps.consumeOIDCState(ctx, in, "github")
		if err != nil {
			return nil, err
		}
		...
		// Gates both new-user creation and existing-user login. Allows
		// all when neither the environment nor the database names an
		// entry; the error path means the allowlist could not be read,
		// which is not the same answer as "this address is not on it".
