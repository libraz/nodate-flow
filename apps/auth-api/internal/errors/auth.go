...
var (
	...
	// AUTH.OIDC.DOMAIN_NOT_ALLOWED — Sign-in is restricted to approved email domains for this instance
	AuthOidcDomainNotAllowed = &Spec{Code: "AUTH.OIDC.DOMAIN_NOT_ALLOWED", Status: 403, Message: "Sign-in is restricted to approved email domains for this instance", Description: "Returned when an OIDC sign-in succeeds at the identity provider but the authenticated account's email domain is not in the instance's allowed sign-in allowlist, so the session is refused.", UserAction: "Sign in with an email address on an approved domain, or ask your instance administrator to add your domain to the allowlist."}
