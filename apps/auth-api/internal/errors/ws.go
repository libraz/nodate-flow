...
var (
	...
	WsTaskActorRoleInvalid = &Spec{Code: "WS.TASK.ACTOR_ROLE_INVALID", Status: 422, Message: "Task actor role must be one of: assignee, reviewer, watcher, approver", Description: "Returned when a task create or update payload includes an `actors[].role` value outside the allowed enum. The persisted column is `task_actors.role ENUM('assignee','reviewer','watcher','approver')`, so any other string is rejected at the validation boundary before the SQL layer.", UserAction: "Set each actor's role to one of: assignee, reviewer, watcher, approver.", I18nKey: "workspace.errors.task_actor_role_invalid"}
